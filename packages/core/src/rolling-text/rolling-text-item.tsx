...
import type { RollingTextDirection } from "./rolling-text.shared"
...
const RollingTextItem: FC<RollingTextItemProps> = (props) => {
  ...
  const newFigureArr = useMemo(
    () => (direction === "down" ? figureArr.slice().reverse() : figureArr),
    [direction, figureArr],
  )
    ...
    >
      ...
      >
        {Array.isArray(newFigureArr) &&
          newFigureArr.map((v, i) => (
            <View
              className={classNames(prefixClassname("rolling-text-item__item"))}
              style={itemStyle}
